...
- **`uv` is documented as an install option alongside `pipx`.** `uv tool install docmancer` puts the CLI in its own isolated tool environment, the same shape pipx provides. Note that `uv pip install` is not equivalent, because it installs into whichever virtualenv is currently active.

- **BREAKING: the three prefixed MCP tools were renamed.** `docmancer_memory_search` is now `search_evidence`, `docmancer_docs_search` is now `search_docs`, and `docmancer_memory_status` is now `evidence_status`. They were the only tools carrying a `docmancer_` prefix, and `docmancer_memory_search` sat one word-order away from `search_memory` while querying an entirely different store, which is the worst possible case for a model choosing between them. The surface now follows two conventions with no prefixes: `<verb>_<noun>` performs an operation, `<noun>_<noun>` returns a named report. Any client calling the old names must update; the tool count is unchanged at 20.
- **BREAKING: alias parameters were removed.** `path`, `content`, `target`, `memory_id`, `hash`, `new_path`, and the `query`/`budget` spellings on `ask_memory` are no longer accepted. Roughly a third of the exposed parameter surface was duplicate spellings, and reconciling them at runtime forced every genuinely required argument to be declared optional, so nine tools advertised nothing as required. Canonical names are now the only spelling, required arguments are typed as required, and a missing or misspelled one fails as a protocol-level schema error instead of an error payload the model has to parse.
- **Every MCP tool description now explains its own parameters, siblings, and return shape.** Parameter descriptions already existed in the JSON Schema, but agents read the description string far more reliably, and tools like `search_docs` shipped a single sentence covering neither their arguments nor how they differed from `search_memory`. Each of the 20 descriptions now states its purpose, side effects and cost, when not to use it and what to use instead, what each parameter means, and what comes back. A new test asserts that every parameter is named in its tool's description, so this cannot silently regress.
- **Tools declare titles, behavioural annotations, and enums.** All 20 now carry a human-readable title; the three renamed tools gained the `readOnlyHint`/`destructiveHint`/`idempotentHint`/`openWorldHint` annotations they previously lacked entirely. Closed value sets are now real schema enums rather than prose: canonical `section` (with pin and unpin correctly excluding the self-describing section), `scope`, `authority`, `ask_memory` `mode`, and `decision_timeline` `operation`.
...
- **Legacy Context-pack browser API.** The old `/api/v1/context` pack mutation collision moved out of the Context artifact namespace. Context revisions now own that route.
...
- **Provider-backed consolidation reliability.** OpenRouter consolidation now uses smaller default batches, a larger output cap, and one structured-output retry when a provider returns malformed JSON. Agent CLI providers now get a longer default timeout so large Codex consolidation batches do not fail at the generic 180 second provider timeout.
