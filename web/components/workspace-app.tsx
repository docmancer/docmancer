...
export type ViewKey = "overview" | "context" | "memory" | "sources" | "docs" | "audit" | "intelligence" | "maintenance" | "sync" | "devices" | "team" | "help";
...
const VIEW_COPY: Record<ViewKey, { eyebrow: string; title: string; description: string }> = {
  overview: { eyebrow: "Local workspace", title: "Your agent memory at a glance", description: "Check what is indexed, what agents can carry, and whether anything needs review." },
  context: { eyebrow: "Prepared context", title: "What your agents should carry", description: "Four durable context packs contain approved statements. Individual memories remain supporting evidence until you promote or distil them." },
  memory: { eyebrow: "Memory atoms", title: "Search what your agents already learned", description: "A memory atom is one source-attributed fact, decision, rule, preference, or workflow. Open one to inspect provenance or change its lifecycle." },
  sources: { eyebrow: "Source files", title: "See where memory came from", description: "Browse the files Docmancer harvested. Open a source to read, edit, reindex, or remove it locally." },
  ...
  intelligence: { eyebrow: "Derived review", title: "Resolve conflict, drift, and noise", description: "Review analysis derived from the memory graph. Nothing here is a separate memory store, and nothing changes without your decision." },
...
function ViewContent(props: ViewProps) {
  ...
  if (props.view === "help") return <HelpView counts={props.counts}/>;
...
function Overview({ data, counts }: { data: JsonMap; counts: JsonMap }) {
  ...
  const cards = [["Memory atoms", displayValue(counts.atoms), "Searchable evidence"], ["Context packs", displayValue(counts.context), "Approved agent context"], ["Source files", displayValue(counts.sources), "Harvested locally"], ["Documentation", displayValue(counts.docs), "Separate reference index"]];
...
function HelpView({ counts }: { counts: JsonMap }) {
  ...
  return <div className="help-layout"><nav className="help-tabs" aria-label="Help sections">{tabs.map(([key, label], index) => <button key={key} className={section === key ? "active" : ""} onClick={() => setSection(key)}><span>{String(index + 1).padStart(2, "0")}</span>{label}</button>)}</nav><section className="help-content">
    {section === "start" && <><div className="help-intro"><span className="mini-label">Your current workspace</span><h2>From scattered files to deliberate agent context</h2><p>Docmancer has indexed <strong>{displayValue(counts.atoms)} memory atoms</strong> from <strong>{displayValue(counts.sources)} source files</strong>. Those atoms are searchable evidence. Your agents carry only the approved statements in Context.</p></div><ol className="guide-steps"><GuideStep number="1" title="Harvest local sources" text="Run Memory sync after agent files change. Docmancer discovers supported memory, instructions, and rules, redacts risky values, and rebuilds the local index." command="docmancer sync --local-only"/><GuideStep number="2" title="Inspect the evidence" text="Use Memory to search individual facts and Sources to understand the files behind them. Editing a source preserves provenance and reindexes the file."/><GuideStep number="3" title="Prepare context" text="Add one statement directly or distil a larger evidence set into a review proposal. Approve only what should become durable agent context." command="docmancer memory distill --into personal-defaults"/><GuideStep number="4" title="Deliver it to agents" text="Installed hooks recall relevant evidence automatically. Managed agent projections receive the approved context packs in a stable order." command="docmancer agent refresh"/></ol></>}
    {section === "workflow" && <div className="help-sections"><HelpSection title="Capture and harvest" text="Memory begins in agent files, instructions, rules, manual additions, imports, or optional capture hooks. Sources shows those original files." links={[["Open Sources", "/sources/"], ["Add a memory", "/memory/"]]}/><HelpSection title="Index and recall" text="A sync extracts source-attributed memory atoms and rebuilds local search and graph relationships. Memory is the searchable evidence layer." links={[["Search Memory", "/memory/"], ["Run Maintenance", "/maintenance/"]]}/><HelpSection title="Review and approve" text="Intelligence analyses conflicts and orphans. Distillation creates proposals for Context. Neither surface silently creates approved memory." links={[["Open Intelligence", "/intelligence/"], ["Open Context", "/context/"]]}/><HelpSection title="Maintain and audit" text="Audit locates masked risks. Maintenance rebuilds the index, drafts consolidation, applies context, and runs diagnostics through allowlisted operations." links={[["Open Audit", "/audit/"], ["Open Maintenance", "/maintenance/"]]}/></div>}
    ...
    {section === "glossary" && <dl className="glossary"><GlossaryTerm term="Memory atom" text="One small, self-contained, source-attributed fact, decision, rule, preference, or workflow."/><GlossaryTerm term="Source" text="The original local file harvested by Docmancer. A source can produce many memory atoms."/><GlossaryTerm term="Context pack" text="A durable ordered set of approved statements compiled for agents. The four defaults cover personal or team scope and global or project applicability."/><GlossaryTerm term="Distillation" text="A reviewable proposal that reconciles relevant evidence into additions, consolidations, removals, or overrides for a context pack."/><GlossaryTerm term="Intelligence" text="Derived graph analysis for conflicts, recent changes, orphans, and resolved history. It is not a separate memory store."/><GlossaryTerm term="Recall" text="Retrieving the most relevant approved context and supporting evidence for a question or agent task."/><GlossaryTerm term="Personal Sync" text="Optional paid transport and recovery for encrypted revisions between your approved devices."/></dl>}
  </section></div>;
}
...
function CommandPalette({ close, run }: { close: () => void; run: (command: string) => Promise<void> }) {
  ...
  const commands = [{ key: "sync", label: "Sync local memory", cli: "docmancer sync --local-only", icon: RefreshCw }, { key: "distill", label: "Propose Personal defaults", cli: "docmancer memory distill --into personal-defaults", icon: Sparkles }, { key: "doctor", label: "Run diagnostics", cli: "docmancer status --check", icon: FileSearch }, { key: "consolidate", label: "Draft a standalone consolidation", cli: "docmancer memory consolidate", icon: WandSparkles }, { key: "cloudSync", label: "Run encrypted sync", cli: "docmancer cloud sync", icon: Cloud }].filter((item) => `${item.label} ${item.cli}`.toLowerCase().includes(needle));
...
function rowKicker(item: JsonMap, view: ViewKey): string { if (view === "memory") return sourceLabel(item); if (view === "context" && item.view_kind === "context-proposal") return "Review proposal"; if (view === "context" && item.view_kind === "context-record") return `Approved statement · ${sourceLabel(item)}`; if (view === "sources") return humanise(String(item.kind ?? "local source")); return ""; }
...
function commandLabel(command: string): string { return ({ sync: "Memory sync", distill: "Context proposal", doctor: "Diagnostics", consolidate: "Consolidation", cloudSync: "Encrypted sync" } as Record<string, string>)[command] ?? command; }
...
function sourceLabel(item: JsonMap): string { const path = String(item.source_path ?? item.source ?? "Local source"); return humaniseFilename(path) || "Local source"; }
