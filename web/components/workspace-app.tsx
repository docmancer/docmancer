...
import {
  Activity, Archive, BookOpen, BrainCircuit, ChevronRight, Cloud, Command, Database,
  FileSearch, Fingerprint, Gauge, KeyRound, LoaderCircle, Moon, Play, Plus, RefreshCw,
  Search, Settings, ShieldCheck, Sparkles, Sun, Users, WandSparkles, X,
} from "lucide-react";
...
function CollectionView(props: ViewProps) {
  ...
  return <div className="workspace-grid"><section className="collection">
    ...
    <div className="rows">{items.length ? items.map((item, index) => <DataRow key={rowKey(item, index)} item={item} view={props.view} inspect={() => void props.inspect(item)}/>) : <EmptyState view={props.view}/>}</div>
    {pages > 1 && <nav className="pagination" aria-label={`${humanise(props.view)} pages`}><button className="secondary" disabled={page <= 1} onClick={() => void props.load(collectionPagePath(props.view, page - 1, props.query))}>Previous</button><span>Page {page} of {pages}</span><button className="secondary" disabled={page >= pages} onClick={() => void props.load(collectionPagePath(props.view, page + 1, props.query))}>Next</button></nav>}
...
function DataRow({ item, view, inspect }: { item: JsonMap; view: ViewKey; inspect: () => void }) {
  ...
  return <article className="data-row interactive-row" onClick={inspect} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); inspect(); } }} role="button" tabIndex={0}><div><h3>{truncate(title, 180)}</h3><span>{subtitle}</span>{text && text !== title && <p>{truncate(text, 360)}</p>}</div><button aria-label={`Open ${truncate(title, 60)}`} onClick={(event) => { event.stopPropagation(); inspect(); }}><ChevronRight size={15}/></button></article>;
}
...
function EmptyState({ view }: { view: ViewKey }) { return <div className="empty"><div><Archive size={24}/></div><h3>No {view} items yet</h3><p>Run the relevant local action or change the search to populate this view.</p></div>; }
...
function rowSubtitle(item: JsonMap, view: ViewKey): string {
  ...
  if (view === "context") return `${String(item.pack_name ?? item.audience_kind ?? item.view_kind ?? "context")} · ${String(item.memory_type ?? item.applicability_kind ?? "")}`;
...
function rowText(item: JsonMap, view: ViewKey): string { if (view === "sources" && Array.isArray(item.matches)) return `${item.matches.length} matching atoms. Open to inspect excerpts and edit the full file.`; if (view === "audit" && item.path) return String(item.error ?? item.path); return String(item.text ?? item.rendered ?? item.detail ?? item.summary ?? ""); }
...
function collectionPagePath(view: ViewKey, page: number, query: string): string { const [base, existing = ""] = ENDPOINTS[view].split("?"); const params = new URLSearchParams(existing); params.set("page", String(page)); if (query.trim()) params.set("q", query.trim()); return `${base}?${params.toString()}`; }
