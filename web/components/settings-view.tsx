...
import {
  ...
  LoaderCircle, RefreshCw, Search, ShieldCheck, SlidersHorizontal, Sparkles,
} from "lucide-react";
...
export function AgentEditor({ onSaved }: { onSaved?: () => void }) {
  ...
  return <div className="settings-form">
    <div className="settings-title"><div className="agent-avatar small"><Sparkles size={20}/></div><div><span className="eyebrow">Your memory agent</span><h2>Customize Docmancer</h2><p>Give Docmancer durable guidance for how it should help you.</p></div></div>
...
export function SetupFlow({ initial, onComplete }: { initial: JsonMap; onComplete?: () => void }) {
  ...
  const connected = items.filter((item) => item.integration_state === "connected");
  const ready = items.filter((item) => item.action_kind === "automatic" && (item.detected || item.integration_state === "stale" || item.integration_state === "partial"));
  const manual = items.filter((item) => item.action_kind === "manual");
  const available = items.filter((item) => !connected.includes(item) && !ready.includes(item) && !manual.includes(item));
  const [selected, setSelected] = useState<Set<string>>(() => new Set(ready.filter((item) => recommended.has(String(item.id))).map((item) => String(item.id))));
  ...
  const connect = async () => {
    setBusy(true); setError("");
    try {
      const result = await apiJobMutation("/api/v1/agent/setup", { targets: [...selected], recall_hooks: recall, capture_hooks: capture }, () => undefined);
  ...
  const hasHookTarget = ready.some((item) => selected.has(String(item.id)) && item.recall_capable);
  return <div className="settings-form setup-flow">
    ...
    {ready.length > 0 && <AgentGroup title="Ready to connect automatically" note="Only the checked integrations will be changed.">
      {ready.map((item) => {
      ...
      <div className="agent-group-action"><button className="primary-btn" disabled={busy || selected.size === 0} onClick={connect}>{busy ? <LoaderCircle className="spin" size={15}/> : <Command size={15}/>}Connect {selected.size} agent{selected.size === 1 ? "" : "s"}</button></div>
    </AgentGroup>}
    ...
    {hasHookTarget && <label className="toggle-row"><input type="checkbox" checked={recall} onChange={(event) => setRecall(event.target.checked)}/><span><strong>Install automatic recall</strong><small>Give supported agents relevant Context at session boundaries.</small></span></label>}
    {hasHookTarget && <label className="toggle-row"><input type="checkbox" checked={capture} onChange={(event) => setCapture(event.target.checked)}/><span><strong>Enable capture hooks</strong><small>Optional and off by default. Capture eligible local session memory.</small></span></label>}
    ...
    <details className="advanced-settings terminal-footer"><summary>Prefer the terminal? <ChevronDown size={15}/></summary><div className="command-stack"><CommandRow title="Detected agents" command="docmancer setup" note="Index memory and install detected integrations."/><CommandRow title="Every supported agent" command="docmancer setup --all" note="Install every supported integration."/></div></details>
  </div>;
}
...
function CloudSettings() {
  ...
  const sync = async () => {
    setBusy(true);
    try { await apiJobMutation("/api/v1/cloud/sync", {}, () => undefined); await load(); }
    catch (reason) { setError(messageOf(reason)); }
    finally { setBusy(false); }
  };
  ...
  if (!status.configured) return <div className="settings-form">
    ...
    <div className="cloud-plan-grid">
      <article><span className="eyebrow">Personal Sync</span><h3>Keep every approved device current</h3><p>Replicate encrypted revisions, recover after a reset, and keep managed history without uploading plaintext memory.</p><a className="secondary-btn" href="https://docmancer.dev/cloud" target="_blank" rel="noreferrer">Explore Personal Sync <ExternalLink size={14}/></a></article>
      <article><span className="eyebrow">Team</span><h3>Share one approved Context file</h3><p>Generate and review the complete file locally, then encrypt it before hosted coordination and delivery.</p><a className="secondary-btn" href="https://docmancer.dev/teams" target="_blank" rel="noreferrer">Explore Team <ExternalLink size={14}/></a></article>
    </div>
    <div className="cloud-pricing-row"><div><strong>Local intelligence stays free</strong><p>Paid plans do not improve local recall quality or unlock local commands.</p></div><a className="primary-btn" href="https://docmancer.dev/pricing" target="_blank" rel="noreferrer">Compare plans <ExternalLink size={14}/></a></div>
    <CommandRow title="Connect after subscribing" command="docmancer cloud connect" note="Authorize this device and keep encryption keys local."/>
  </div>;
  ...
  return <div className="settings-form">
    ...
    {error && <Notice kind="error">{error}</Notice>}
    <div className="cloud-status-grid"><article><strong>{String(status.plan ?? status.entitlement ?? "Connected")}</strong><span>Current plan</span></article><article><strong>{devices.length}</strong><span>Approved devices</span></article><article><strong>{members.length}</strong><span>Team members</span></article></div>
    <div className="form-actions"><button className="primary-btn" disabled={busy} onClick={sync}>{busy && <LoaderCircle className="spin" size={14}/>}Sync now</button><a className="secondary-btn" href="https://docmancer.dev/account" target="_blank" rel="noreferrer">Account and billing <ExternalLink size={14}/></a></div>
  </div>;
}
