...
export function SetupFlow({ initial, onComplete }: { initial: JsonMap; onComplete?: () => void }) {
  ...
  if (reviewing) {
    ...
    return <div className="settings-form setup-flow setup-confirmation">
      ...
      <Notice>Canonical plaintext stays on this machine. If you configured an AI provider, redacted evidence is sent to it for synthesis on every rebuild, which spends tokens on your key. Without a provider, Docmancer builds the same sections locally and deterministically. Existing agent files are updated through bounded Docmancer-managed blocks.</Notice>
  ...
  return <div className="settings-form setup-flow">
    ...
    {hasHookTarget && <div className="setup-default"><Check size={15}/><span><strong>Automatic memory is included</strong><small>Supported agents capture durable conclusions, share one machine-wide canonical memory, and recall it at session and prompt boundaries.</small></span></div>}
