...
function render(bundle) {
  const items = [...(bundle.mandatory_policies || []), ...(bundle.curated_memory || []), ...(bundle.relevant_evidence || [])];
...
export default definePluginEntry({
  ...
  register(api) {
    api.on("before_prompt_build", async (event, ctx) => {
      ...
      try {
        const { stdout } = await run("docmancer", ["ask", task.slice(0, 8000), "--project", cwd, "--token-budget", String(budget), "--json"], { cwd, timeout: 3000, maxBuffer: 1024 * 1024 });
